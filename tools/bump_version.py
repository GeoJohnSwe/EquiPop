...
NOWHERE IT MUST NOT.
    ...
    python tools/bump_version.py 1.47.6
    python tools/bump_version.py 1.47.6 --check   # say, change nothing

WHY THIS EXISTS (v1.47.6). The version lived in a dozen files and was
moved with a blanket sed:

    for f in $(grep -rl "1.47.5" .); do sed -i 's/1.47.5/1.47.6/g' $f; done
...
def main(argv):
    ...
    if not re.fullmatch(r"\d+\.\d+\.\d+", new):
        print(f"[bump] '{new}' is not a version like 1.47.6")
        return 2
    check = "--check" in argv
    old = current()
    if old == new:
