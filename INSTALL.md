...
1. **`--no-deps`, always.** Without it pip upgrades the host's numpy,
   ...
   **AND IT LEAVES A HOLE THAT NOBODY WROTE DOWN UNTIL 1.47.7.**
...
3. Install:
   ```
   python -m pip install --user --no-deps --force-reinstall "C:\path\to\equipop-1.47.7-py3-none-any.whl"
   ```
4. Verify (see below), then **restart QGIS**.
5. Plugin: **Plugins → Manage and Install Plugins → Install from ZIP**,
   choose `equipop_qgis-1.47.7.zip`, then restart QGIS again.
...
```
shell C:\Users\...\python.exe -m pip install --no-deps --force-reinstall C:\path\to\equipop-1.47.7-py3-none-any.whl
...
subprocess.run([py, "-m", "pip", "install", "--no-deps",
                "--force-reinstall",
                r"C:\path\to\equipop-1.47.7-py3-none-any.whl"], env=env)
```
...
with `"equipop==1.47.7"` and drop `--no-deps --force-reinstall`.
