...
| 1.16.0 | #21d - the toolbox completes its ANALYSIS FAMILY: Tool 3 HOTSPOTS (LISA / local Moran's I on any field - typically an EquiPop result - via dispatch engine 'lisa'; quad codes 1-4 and the read-quad-WITH-p rule taught in the dialog) + Tool 4 ACCESSIBILITY (2SFCA/3SFCA: demand layer meets supply layer; reach dials decay-half-life / fixed radius / k-nearest-SUPPLY self-reveal via updateParameters; outputs A_<supply> = competition-included access and J_<supply> = competition-blind potential, both explained at runtime) + Tool 5 FEATURES TO BARRIERS - the promised features_to_friction Pro wrapper, geopandas-FREE: new package rasterizer `paths_to_friction` (Liang-Barsky segment clip; Sutherland-Hodgman ring clip with signed areas, holes honoured) validated CELL-FOR-CELL against the shapely twin on random lines and polygons; the twin cross-check FOUND a latent v1.15 bug (polygons billed for boundary-LENGTH contact against the stated positive-measure convention) - fixed in features_to_friction; dispatcher: fca accepts a DataFrame supply (GIS layers need no temp file) and the parked k_side option now flows through (A_ksupply/A_kdemand reach rows); stub arcpy grew named layers, per-layer shape types and a geometry SearchCursor. 72 tests |
...
### Validation record - v1.16.0 (#21d)
- LISA glue verbatim under fake arcpy: planted 5x5 hotspot -> quad HH with p <= 0.05 in the core, quiet far field; Ii and p IDENTICAL to dispatch('lisa') (seeded permutations) (pytest).
- FCA glue verbatim, two layers: demand-weighted total access returns the WHOLE supply under unbounded decay reach (2SFCA conservation, < 1e-6); A/J identical to dispatch('fca'); Null propagation; k-supply reach re-run over Overwrite (pytest).
- `paths_to_friction` == `features_to_friction` CELL-FOR-CELL: 15 random polylines and 9 polygons incl. a holed rectangle (pytest); conventions pinned numpy-side: corner-kiss free, polygon edge-contact free, one charge per feature per cell, additive stacking 6+4=10.
- Tool 5 verbatim: fake Pro geometries (Polyline with value field; Polygon with None-separated hole ring, default value) -> csv equal to the shapely rasterizer frames; point layers refused loudly (pytest).
- dispatch fca: DataFrame supply == csv supply to 1e-12; k_side='both' yields A_ksupply/A_kdemand row-aligned (pytest).
- FOUND BY THE TWIN: features_to_friction charged polygons for zero-area boundary contact (intersection length > 0). Fixed to the stated rule - length for LINE features, area for POLYGON features.

### Design decisions - v1.16.0
- Cross-implementation twins are VALIDATORS, not redundancy: writing the numpy rasterizer to the written convention exposed that the shapely original had drifted from it. Keep both; test them against each other forever.
- LISA and FCA enter Pro as ANALYSES (tools 3-4) while friction stays an INGREDIENT (tool 1) - the v1.15 taxonomy holds: rulers compose, analyses conclude.
- FCA field names carry the supply field (A_slots, J_jobs): two accessibility runs against different supplies coexist on one layer without collision.
- Tool 5 emits a CSV, not fields: a barrier table is an INPUT artifact shared across runs and projects, so it lives as a file beside the data, in the exact shape tool 1 already accepts.
- Version-history housekeeping: the 1.15.0 row had been duplicated (top block + descending block); the top slot now carries 1.16.0, the descending block keeps 1.15.0 once.
